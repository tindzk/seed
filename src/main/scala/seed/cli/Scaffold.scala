...
class Scaffold(log: Log, silent: Boolean = false) {
  ...
  def askModuleName(): String = {
    log.info(
      s"${Ansi.italic("Module name?")} [default: ${Ansi.underlined("example")}]"
    )
    readInput(
      "example",
      x =>
  ...
  def askStable(): Boolean = {
    log.info(
      s"${Ansi.italic("Do you want to use:")} ${Ansi.bold("1)")} stable releases or ${Ansi
        .bold("2)")} pre-releases? [default: ${Ansi.underlined("1")}]"
    )
    readInput[Boolean](
      true,
      input =>
  ...
  def askOrganisation(): Organisation = {
    log.info(s"${Ansi.italic("Do you want to use:")} ${Ansi
      .bold("1)")} Lightbend or ${Ansi.bold("2)")} Typelevel Scala (${Ansi
      .italic("legacy")})? [default: ${Ansi.underlined("1")}]")
    readInput[Organisation](
  ...
  type Libraries = Map[Artefact, List[
  ...
  /** Find platform configuration supporting one of the compiler versions.
    ...
    */
  def choosePlatformConfiguration(
    ...
    platforms.toList
      ...
      .flatMap { platform =>
        ...
        val versionTuplesForPlatform = versionTuples.filter {
          case (_, compilerVersion) =>
              ...
              versionTuplesForPlatform.reverse.maxBy {
                case (platformVersion, compilerVersion) =>
                  libraries.count {
                    case (_, versions) =>
                      versions
                        .contains((platform, platformVersion, compilerVersion))
                  }
              }
          )
      }
  ...
  def fetchCompilerVersions(
    ...
    val compilerArtefacts: Map[Platform, Artefact] =
      (if (platforms.contains(JavaScript))
         Map((JavaScript, Artefact.ScalaJsCompiler))
       else Map()) ++
        (if (platforms.contains(Native))
           Map((Native, Artefact.ScalaNativePlugin))
         else Map())

    Map((JVM, organisationVersions)) ++
      compilerArtefacts.map {
        case (platform, artefact) =>
          val versions =
  ...
  def libraryPlatformCompatibility(
    platforms: Set[Platform],
  ...
  def checkVersions(
    ...
    val compilerVersions =
      fetchCompilerVersions(organisation, platforms, stable)

    val platformVersions: Map[Platform, (PlatformVersion, CompilerVersion)] =
      if (artefacts.isEmpty)
        Map[Platform, (PlatformVersion, CompilerVersion)]()
      else
        libraryPlatformCompatibility(
          platforms,
          artefacts.flatMap(_._2).toSet,
          compilerVersions,
          stable
        )
    ...
    val resolvedPlatformAndCompilerVersions
      : Map[Platform, (PlatformVersion, CompilerVersion)] =
      platforms.toList
        .sorted(Platform.Ordering)
        .flatMap { platform =>
          compilerVersions.get(JVM).flatMap { jvmVersions =>
            ...
            // exists
            val versions = jvmVersions
              .filter(cv => bridgeVersions.exists(bv => cv.startsWith(bv)))

            val platformAndCompilerVersion = platformVersions.get(platform)
            val (platformVersion, suitableCompilerVersions) =
              platformAndCompilerVersion match {
                case None => ("", versions.reverse)
                case Some((pv, cv)) =>
                  (
                    pv,
                    versions
                      .filter(v => MavenCentral.trimCompilerVersion(v) == cv)
                      .reverse
                  )
              }

            suitableCompilerVersions.view
              .flatMap { cv =>
                if (platform == JavaScript) {
                  ...
                  versions.reverse
                    .find(_.startsWith(platformVersion))
                    .map(_ -> cv)
    ...
    println(
      util.Tabulator.format(
        List(
          List("Platform", "Organisation", "Compiler", "Version")
            .map(fansi.Bold.On(_))
            ...
            if (platform == JavaScript)
              Some(
                Item(
                  "Scala.js",
                  "Plug-in",
                  resolvedPlatformVersions.get(platform)
                )
              )
            else if (platform == Native)
              Some(
          ...
          allArtefacts.zipWithIndex.map {
            case (Item(org, compiler, version), i) =>
              List(
                if (i != 0) fansi.Str("")
                ...
                version match {
                  ...
                  case Some(_) =>
                    ColourScheme.green1.toFansi(fansi.Str(compiler))
                },
                version match {
                  case None =>
                    ColourScheme.red1.toFansi(fansi.Str("Not available"))
    ...
    val libraryArtefacts =
      if (!artefacts.exists(_._2.nonEmpty))
        ...
        println(
          util.Tabulator.format(
            List(
              List("Platform", "Organisation", "Artefact", "Version")
                .map(fansi.Bold.On(_))
            ) ++ libraryArtefacts.toList
              .sortBy(_._1)(Platform.Ordering)
              .flatMap {
                case (platform, artefacts) =>
                  // Keep toList, otherwise indices may be out of order
                  artefacts.toList.zipWithIndex.map {
                    case ((artefact, version), i) =>
                      List(
                        if (i != 0) fansi.Str("")
                        else
                          ColourScheme.yellow2
                            .toFansi(fansi.Str(platform.caption)),
                        version match {
                          case None =>
                            ColourScheme.red1
                              .toFansi(fansi.Str(artefact.organisation))
                          case Some(_) =>
                            ColourScheme.green1
                              .toFansi(fansi.Str(artefact.organisation))
                        },
                        version match {
                          case None =>
                            ColourScheme.red1.toFansi(fansi.Str(artefact.name))
                          case Some(_) =>
                            ColourScheme.green1
                              .toFansi(fansi.Str(artefact.name))
                        },
                        version match {
                          case None =>
                            ColourScheme.red1
                              .toFansi(fansi.Str("Not available"))
                          case Some(v) =>
                            ColourScheme.green1.toFansi(fansi.Str(v))
                        }
                      )
                  }
              }
  ...
  def generateBuildFile(
    ...
    platforms: Set[Platform],
    testFrameworks: Set[TestFramework]
  ): toml.Root = {
    ...
    def scalaTestDeps(platform: Platform) = {
      val frameworks = testFrameworks
        .flatMap(
          tf =>
            resolveLibraryVersion(platform, tf.artefact).toList.map(
              version =>
                Arr(
                  List(
  ...
  def ui(path: Path): Unit = {
    ...
    if (Files.exists(seedPath))
      log.error(
        s"The file ${italic(seedPath.toString)} exists already. Please provide a different path."
      )
    else
      create(seedPath)
  }
